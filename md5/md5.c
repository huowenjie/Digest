...
{
    if (!md5_calc) {
        ERROR_LOCATE("FUNCTION POINTER IS NULL!");
        return 0;
    }
...
{
    if (!ctx) {
        ERROR_LOCATE("NULL_POINTER MD5_CTX!");
        return FAILED;
    }
...
{
    if (!ctx) { ERROR_LOCATE("NULL_POINTER MD5_CTX!"); return FAILED; }
    if (!msg) { ERROR_LOCATE("NULL_POINTER MD5 MSG!"); return FAILED; }
...
int md5_msg(const uint8_t *in_msg, uint32_t in_len, uint8_t *out_msg, uint32_t *out_len)
{
    ...
    ret = md5_init(&ctx);
    if (ret != SUCCESS) {
        ERROR_LOCATE("md5_init ERROR!"); 
        return ret;
    }

    ret = md5_update(&ctx, in_msg, in_len);
    if (ret != SUCCESS) {
        ERROR_LOCATE("md5_update ERROR!"); 
        return ret;
    }

    ret = md5_final(&ctx, out_msg, out_len);
    if (ret != SUCCESS) {
        ERROR_LOCATE("md5_final ERROR!"); 
    }
